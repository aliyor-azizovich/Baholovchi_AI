...
class UkupWidget(QWidget):
    def __init__(self, parent=None, main_window=None, valuation_window = None):
        ...
        uic.loadUi(os.path.join(os.path.dirname(__file__), "ukup_window.ui"), self)
        ...
        self.tableWidget_liter_list.setHorizontalHeaderLabels([
            "№", "Здание", "Восст. стоимость здания",
            "Усреднённый износ", "Оценочная стоимость"
        ])
...
# СОХРАНЯЕМ ДАННЫЕ
    ...
    def collect_liter_base_data(self):
        ...
        for name, combo in self.combo_boxes.items():
            if combo.isEnabled() and combo.currentText() and combo.currentText() != "Выберите из списка":
                filters[name] = combo.currentText()
