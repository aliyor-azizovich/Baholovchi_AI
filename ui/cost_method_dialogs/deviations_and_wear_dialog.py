...
class DeviationsAndWearDialog(QDialog):
    def __init__(self, parent=None, data_service=None, valuation_window=None):
        ...
        uic.loadUi(os.path.join(os.path.dirname(__file__), "deviations_and_wear_dialog.ui"), self)
        self.parent = parent
    ...
    def set_seismic_correction(self):
        try:
            ...
            # Получаем выбранную область из интерфейса
            selected_region = self.valuation_window.comboBox_oblast.currentText()

            # Фильтруем по столбцу 'region'
            match = df[df['region'] == selected_region]

            if not match.empty:
                # Присваиваем значение столбца 'sesmos_correction' переменной self.sesmos
    ...
    def populate_wear_table(self, df):
        ...
        self.tableWidget_wear.setHorizontalHeaderLabels(df.columns)
